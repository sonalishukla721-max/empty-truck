...
-- ============================================================
-- empty-truck-ai  —  Supabase Schema  (IDEMPOTENT — safe to re-run)
-- ============================================================

-- ─── DROP ALL TABLES (reverse dependency order) then recreate cleanly ────────
-- This avoids CASCADE breaking column types on existing tables
DROP TABLE IF EXISTS public.pilot_validation CASCADE;
DROP TABLE IF EXISTS public.ratings CASCADE;
DROP TABLE IF EXISTS public.notifications CASCADE;
DROP TABLE IF EXISTS public.location_updates CASCADE;
DROP TABLE IF EXISTS public.payments CASCADE;
DROP TABLE IF EXISTS public.trips CASCADE;
DROP TABLE IF EXISTS public.return_load_opportunities CASCADE;
...
DROP POLICY IF EXISTS "trucks_read_all" ON public.trucks;
CREATE POLICY "trucks_read_all" ON public.trucks FOR SELECT USING (true);
DROP POLICY IF EXISTS "trucks_write_own" ON public.trucks;
...
CREATE INDEX IF NOT EXISTS idx_trucks_pos ON public.trucks(current_lat, current_lng);
DROP TRIGGER IF EXISTS trucks_updated ON public.trucks;
...
CREATE INDEX IF NOT EXISTS idx_loads_pickup_time ON public.loads(pickup_time);
CREATE INDEX IF NOT EXISTS idx_loads_pickup_pos ON public.loads(pickup_lat, pickup_lng);
DROP TRIGGER IF EXISTS loads_updated ON public.loads;
...
CREATE TABLE IF NOT EXISTS public.return_load_opportunities (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
DROP POLICY IF EXISTS "rlo_read_all" ON public.return_load_opportunities;
CREATE POLICY "rlo_read_all" ON public.return_load_opportunities FOR SELECT USING (true);
DROP POLICY IF EXISTS "rlo_write_auth" ON public.return_load_opportunities;
CREATE POLICY "rlo_write_auth" ON public.return_load_opportunities FOR INSERT TO authenticated WITH CHECK (true);
DROP POLICY IF EXISTS "rlo_update_auth" ON public.return_load_opportunities;
CREATE POLICY "rlo_update_auth" ON public.return_load_opportunities FOR UPDATE TO authenticated USING (true) WITH CHECK (true);
CREATE INDEX IF NOT EXISTS idx_rlo_truck ON public.return_load_opportunities(truck_id);
CREATE INDEX IF NOT EXISTS idx_rlo_score ON public.return_load_opportunities(match_score DESC);
...
DROP POLICY IF EXISTS "trips_write_auth" ON public.trips;
...
DROP POLICY IF EXISTS "payments_party" ON public.payments;
...
DROP POLICY IF EXISTS "loc_read_all" ON public.location_updates;
CREATE POLICY "loc_read_all" ON public.location_updates FOR SELECT USING (true);
DROP POLICY IF EXISTS "loc_insert_owner" ON public.location_updates;
...
CREATE INDEX IF NOT EXISTS idx_loc_truck ON public.location_updates(truck_id);
CREATE INDEX IF NOT EXISTS idx_loc_time ON public.location_updates(timestamp DESC);
...
-- PILOT VALIDATION
CREATE TABLE IF NOT EXISTS public.pilot_validation (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
DROP POLICY IF EXISTS "pilot_read_all" ON public.pilot_validation;
...
-- realtime (safe: only add if not already present)
DO $$
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_publication_tables WHERE pubname='supabase_realtime' AND tablename='loads') THEN
    ALTER PUBLICATION supabase_realtime ADD TABLE public.loads; END IF;
...
DROP POLICY IF EXISTS "bookings_update_demo" ON public.bookings;
